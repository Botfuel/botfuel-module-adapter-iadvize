...
/**
 * Get transfer distributionRules for the operator provided
 * @param operator
 * @returns {Array}
 */
const getOperatorTransferRules = (operator) => {
  ...
  const filteredRules = distributionRules.filter(rule => availabilityStrategy.distributionRulesToCheck.indexOf(rule.id) !== -1);
  logger.debug('getOperatorTransferRules: filtered rules', filteredRules);
